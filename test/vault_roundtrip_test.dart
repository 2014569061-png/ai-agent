...
void main() {
  ...
  test('buildVaultJson -> restoreVault round-trips data and secrets', () async {
    ...
    final sourceStore = _FakeStore(configs: const [
      ProviderConfig(
          ...
          model: 'gpt-4o-mini',
          apiKey: 'sk-test',
          inputPricePerMillionCents: 25,
          outputPricePerMillionCents: 100,
          cachedPricePerMillionCents: 5),
    ], toolKeys: {
      'tavily': 'tvly-123',
    });

    final json = await buildVaultJson(dbA,
        providerStore: sourceStore, includeSecrets: true);
    expect(json['version'], 2);
    final providerSection = json['providerProfiles'] as List;
    expect((providerSection.first as Map)['apiKey'], 'sk-test');
    expect((providerSection.first as Map)['inputPricePerMillionCents'], 25);
    expect((providerSection.first as Map)['outputPricePerMillionCents'], 100);
    expect((providerSection.first as Map)['cachedPricePerMillionCents'], 5);
    expect(((json['toolKeys'] as Map))['tavily'], 'tvly-123');

    final safeJson = await buildVaultJson(dbA, providerStore: sourceStore);
    expect(safeJson.containsKey('providerProfiles'), isFalse);
    expect(safeJson.containsKey('toolKeys'), isFalse);
    ...
    expect(restoreStore.restored!.single.inputPricePerMillionCents, 25);
    expect(restoreStore.restored!.single.outputPricePerMillionCents, 100);
