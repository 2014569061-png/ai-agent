...
Archive _archive() {
  ...
  archive.addFile(ArchiveFile.bytes('repo-main/SKILL.md', utf8.encode(_skillMd)));
  return archive;
}
...
void main() {
  ...
  test('installs a well-formed archive', () async {
    final archive = _archive()
      ..addFile(ArchiveFile.bytes('repo-main/assets/guide.txt', utf8.encode('说明')));
  ...
  test('rejects path traversal entries', () async {
    final archive = _archive()
      ..addFile(
          ArchiveFile.bytes('repo-main/../../evil.md', utf8.encode('x')));
    expect(
      () => installer.previewFromArchiveBytes(_tarGz(archive), _ref()),
      throwsA(predicate((e) =>
          e is SkillValidationException && e.message.contains('路径穿越'))),
    );
  });
  ...
  test('rejects symbolic links', () async {
    ...
    expect(
      () => installer.previewFromArchiveBytes(_tarGz(archive), _ref()),
      throwsA(predicate((e) =>
          e is SkillValidationException && e.message.contains('符号链接'))),
    );
  });

  test('rejects files outside the extension whitelist', () async {
    final archive = _archive()
      ..addFile(ArchiveFile.bytes('repo-main/evil.sh', utf8.encode('rm -rf /')));
    expect(
      () => installer.previewFromArchiveBytes(_tarGz(archive), _ref()),
      throwsA(predicate((e) =>
  ...
  test('rejects archives without SKILL.md', () async {
    ...
    expect(
      () => installer.previewFromArchiveBytes(_tarGz(archive), _ref()),
      throwsA(predicate(
          (e) => e is SkillValidationException && e.message.contains('SKILL.md'))),
    );
  });
}
