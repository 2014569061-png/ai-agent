...
void main() {
  ...
  testWidgets('存量回归：show(context, x) 渲染无图标、无 chip；pump(3s) 后 OverlayEntry 已移除', (WidgetTester tester) async {
    ...
    await tester.pump(const Duration(milliseconds: 300)); // wait for reverse animation

    expect(find.text('test message'), findsNothing);
  });

  testWidgets('error 常驻：error(...) 后 pump(5s) 仍在；图标为 error_outline_rounded、着色 AppTheme.danger；仅存在 × 与复制 chip', (WidgetTester tester) async {
    await tester.pumpWidget(buildApp(Builder(
      builder: (context) {
        return ElevatedButton(
    ...
    expect(find.text('出错了，可重试；详情见技术细节。'), findsOneWidget); // fallback humanize string
    ...
    expect(find.text('出错了，可重试；详情见技术细节。'), findsOneWidget);
  });

  testWidgets('关闭路径：tap × -> entry 移除；tap chip -> onPressed 触发且默认关闭', (WidgetTester tester) async {
  ...
  testWidgets('点胶囊降级：persistent 时 tap 胶囊空白区不关闭', (WidgetTester tester) async {
    ...
    // tap on the message
    await tester.tap(find.text('出错了，可重试；详情见技术细节。'));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 300));
  ...
  testWidgets('替换策略：连续 show 两次 -> overlay 中仅 1 个 entry，且参数为新的一组', (WidgetTester tester) async {
    await tester.pumpWidget(buildApp(Builder(
      builder: (context) {
        return Column(
          children: [
    ...
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 300)); // wait for old toast reverse anim to finish (220ms)
  ...
  testWidgets('tone 映射表：4 个 tone -> icon/颜色断言', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(buildApp(Builder(
      builder: (context) {
        return ElevatedButton(
          onPressed: () => FloatingToast.show(context, 'neutral', tone: ToastTone.neutral),
    ...
    await tester.pumpWidget(buildApp(Builder(
      builder: (context) {
        return ElevatedButton(
          onPressed: () => FloatingToast.show(context, 'success', tone: ToastTone.success),
    ...
    await tester.pumpWidget(buildApp(Builder(
      builder: (context) {
        return ElevatedButton(
          onPressed: () => FloatingToast.show(context, 'warning', tone: ToastTone.warning),
    ...
    await tester.pumpWidget(buildApp(Builder(
      builder: (context) {
        return ElevatedButton(
          onPressed: () => FloatingToast.show(context, 'danger', tone: ToastTone.danger),
  ...
  testWidgets('Semantics：error 胶囊带 liveRegion；× 有 tooltip/label', (WidgetTester tester) async {
    await tester.pumpWidget(buildApp(Builder(
      builder: (context) {
        return ElevatedButton(
    ...
    expect(semantics.properties.label, '出错了，可重试；详情见技术细节。');
