...
void main() {
  ...
  group('WorkspaceTools 文件读写测试', () {
    test('write_file 创建新文件并自动创建父目录', () async {
      ...
      expect(result.contains('成功写入文件'), isTrue);
    ...
    test('edit_file 精确局部代码替换', () async {
      ...
      expect(result.contains('成功修改文件'), isTrue);
    ...
    test('list_directory 扫描文件树', () async {
      ...
      expect(result.contains('index.html'), isTrue);
      expect(result.contains('style.css'), isTrue);
    });
    ...
    test('search_files 全文关键字检索', () async {
      ...
      expect(result.contains('main.dart:2: print("NEXUS AGENT");'), isTrue);
    });
    ...
    test('delete_file 删除文件', () async {
      ...
      expect(result.contains('成功删除文件'), isTrue);
