...
| `AppPalette.textFaint` | 色彩对比度核验 | $\ge 4.5:1$ (普通文本) / $\ge 3:1$ (大文本) | 需结合不同屏幕面板及深浅模式真机校色 | ⚠️ **待 D 真机回归实测** |
...
1. **`textFaint` 灰阶在室外强光与低亮度下的可读性**
   - 浅色模式 `#8B90A0` 在白底 `#FFFFFF` 上的对比度约为 3.1:1（主要用于辅助副标 11px/12px 与分类标）；
   - 深色模式 `#7C8298` 在深色背景 `#0F121C` / `#181D2A` 上的对比度约为 4.6:1；
   - **待 D 真机回归实测**：确认在中低端 LCD/OLED 真机上，`textFaint` 在辅助信息场景下的实际辨识度，必要时微调至 `#757B8D`。
