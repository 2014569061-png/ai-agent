...
class OrchestrationService implements OrchestrationModule {
  ...
  Future<CollaborationRunSnapshot> start(
    ...
    final agents = proposal.agents.take(bounded.maxAgents).toList(growable: false);
  ...
  Future<void> _execute(
      CollaborationRun run, CollaborationPlan plan, DevelopmentTaskInput input) async {
    final db = await databaseFuture;
    ...
    _emit(run.id, CollaborationRunEvent(run.id, _snapshot(current, plan: plan)));

    var context = _contextFor(input);
    final outputs = <CollaborationAgentOutput>[];
    final slots = (plan.agents.length * plan.maxRounds) + 1;
    final perAgentBudget = (plan.budgetTokens ~/ slots).clamp(200, 2000).toInt();
    for (var round = 1; round <= plan.maxRounds; round++) {
      ...
      final pending =
          <Future<(String, CollaborationAgentRun, CollaborationAgentOutput)> Function()>[];
      for (final spec in plan.agents) {
        ...
        final agentRun = CollaborationAgentRun(
          ...
          inputDigest:
              HeadlessCollaborationAgentRunner.digest('$context\n${input.prompt}'),
          outputSummary: null,
          failureReason: null,
          inputTokens: 0,
          outputTokens: 0,
        ...
        _emit(run.id, CollaborationAgentEvent(
      ...
      final completedRuns = <(String, CollaborationAgentRun, CollaborationAgentOutput)>[];
      ...
      for (final item in completedRuns) {
        ...
        await db.saveCollaborationRun(current);
        _emit(run.id, CollaborationAgentEvent(run.id, agentRunId,
            CollaborationAgentRoleX.parse(item.$2.role),
            completed.status, _truncate(output.text, 4000)));
      }
      if (roundOutputs.isNotEmpty && round < plan.maxRounds) {
        context = '$context\n\n上一轮结构化摘要：\n${_truncate(
            roundOutputs.map((item) => '${item.role.wireName}: ${item.text}').join('\n'),
            16000)}';
      }
    }
    if (token.isCancelled) return;
    ...
    await TaskService().updateProgress(db, input.taskId,
        ...
        extra: {'collaborationRunId': run.id, 'confidence': result.confidence});
    _emit(run.id, CollaborationRunEvent(run.id, _snapshot(finished, plan: plan, result: result)));
  }
